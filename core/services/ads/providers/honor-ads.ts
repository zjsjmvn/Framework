...
import { load } from '../../../../../../../extensions/build-extension/@types/packages/builder/@types/public/build-plugin';
/**
 * 激励广告播放失败代码翻译
 */
...
export default class HonorAds implements IAdProvider {
    ...
    public showInterstitial(posName: string): Promise<ShowInterstitialAdCallBackMsg> {
        return new Promise((resolve, reject) => {
            ...
            if (!!bundle) {
                ...
                error(`>> HonorAds::showInterstitial 无法找到posName=${posName}的广告`);
                let msg = new ShowInterstitialAdCallBackMsg();
                msg.success = false;
    ...
    private createInterstitialAdsWithBundle(bundle: InterstitialAdBundle) {
        if (!!window.qg && !!window.qg.createInterstitialAd) {
            ...
            bundle.interstitialInstance = qg.createInterstitialAd({
    ...
    showRewardVideo(posName: string): Promise<ShowRewardVideoCallBackMsg> {
        return new Promise((resolve, reject) => {
            ...
            let msg = new ShowRewardVideoCallBackMsg();
            if (bundle) {
                console.log(">> HonorAds::showRewardVideo");
                if (!!bundle.rewardVideoInstance) {
                    ...
                    bundle.rewardVideoInstance.show().then(() => {
                        console.log('>> HonorAds 广告显示成功');
                        bundle.hasRewardVideoInCache = false;
                    }).catch((err) => {
                        console.error('>> HonorAds::showRewardVideo 广告组件出现问题', JSON.stringify(err));
                        msg.success = false;
                    ...
                    error(`>> HonorAds::rewardedVideoAd rewardVideoInstance为空`);
                    this.isShowingRewardVideo = false;
                    msg.success = false;
                ...
                error(`>> HonorAds::rewardedVideoAd 无法找到posName=${posName}的广告`);
                msg.success = false;
                this.isShowingRewardVideo = false;
                msg.errMsg = `无法找到posName=${posName}的广告`;
                resolve(msg);
        ...
        this.rewardVideoInstanceMap.forEach((value, key) => {
            if (!!value) {
                value.rewardVideoInstance.load()
                    .then(() => {
                        console.log(`HonorAds ${key}拉取视频广告成功`);
                        value.hasRewardVideoInCache = true;
                    })
                    .catch(() => {
                        console.log(`HonorAds ${key}拉取视频广告失败`);
    ...
    showBanner(posName: string): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            if (bundle && !bundle.bShow) {
                ...
                error(`>> HonorAds::showBanner 无法找到posName=${posName}的广告`);
                resolve(false);
            }
        })
    }
    ...
    public showGeZi(posName: string, style?: {
        left?: number,
        top?: number,
        ...
        if (bundle) {
            if (window.qg && window.qg.createCustomAd) {
                ...
                // @ts-ignore
                bundle.geZiInstance = window.qg.createCustomAd(param);
