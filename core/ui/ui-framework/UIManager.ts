...
export default class UIManager {
     ...
     * 挂起当前展示中的弹窗
     */
    private async suspendCurrentUI() {
        if (!this._currentShowingPopup) {
            return;
        }
        // 从showingStack移除
        ...
        // @ts-ignore
        await this._currentShowingPopup.node.getComponent(UIBase).onSuspended();
        // 关闭当前弹窗（挂起）
        // @ts-ignore
        await this._currentShowingPopup.node.getComponent(UIBase).hide();
        // 置空当前
